...
class AssertionUtil {   
    ...
    static assertTextMatches(actualText, expectedText) {
        expect(actualText).toBe(expectedText, `Expected text "${expectedText}" not found in "${actualText}"`);
    } 

    static assertTextContains(actualText, expectedText) {
        expect(actualText).toContain(expectedText, `Expected text "${expectedText}" not found in "${actualText}"`);
    } 
    ...
    static async assertTitleContains(expectedTitle) {
        const currentTitle = await browser.getTitle();
        expect(currentTitle).toContain(expectedTitle, `Expected title "${expectedTitle}" does not match "${currentTitle}"`);
    }
}
